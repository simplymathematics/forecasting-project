...
All of our models performed fairly well, with a measured RMSE of less than one-tenth of a pH point. Random Forecast outperformed other models with the smallest RMSE and largest RSquared values in training (0.0410 and 0.96) and test (0.099 and 0.684), and is selected as a winner. Random Forecast is a tree-based model, it is hard to get coefficients for each variables, but it does have a way to show the importance of the variables in affecting the target variables, as showed in the important variable lists above. The top 6 highly important variables are Mnf.FLow, Band.Code_C(via one-hot-encoding of Brand.Code), Pressure.Vacuum, Oxygen.Filler, and Temperature. By changing these variables, we can maximum control the PH of the product.

Even though Random Forest regressor has the worst split between test and train data which is symptomatic of over fitting. However, this split was greatly reduced by gradient boosting, at the cost of higher RMSE. Additionally, the support vector machine also suffered from a test/train split that was fairly large. Therefore, it is hard to know how it will perform across a generalized data set. Since the GLM underperforms the only remaining model (the neural network) by only .01 RMSE, there seems to be no downsides to deploying the linear model. In additional, the computational costs associate with the neural network far outweigh the marginal benefits. A linear model can be computed trivially on any type of hardware (CPU or GPU). This is the method we will be deploying.


# Next Step

They are many ways we can continue improving the model performance, one method could be running more times of cross validation on more folds than 3 times 3-fold we have now for SVM and GBM models, it will take a longer time to compute, but the results will be lifted. Another one is to try different model training and test split, as the big differences are showed in training and test model performance in Random Forecast and SVM.
