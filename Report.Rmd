...
Below we can see a sample of the data, as read from Excel. Our target value is pH and each data vector is numeric, except for brand code. However, we can convert this is a numeric vector easily. There are also some missing values that must be imputed.
```{r}
...
```{r}
training <- as.data.frame(sapply(training[- Brand], as.numeric))
